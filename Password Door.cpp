...
bool tiny2D_Init() {
    ...
    passwordStr = to_string(numberOne) + to_string(numberTwo) + to_string(numberThree);
...
void _ROOM(float dt) {
    ...
    DrawText2D("Enter the 3-digit code to open the door:", 10, 100, color.BLACK);
    ...
    if (inputCode.size() > 3) {
        inputCode = inputCode.substr(0, 3);
    }
}
